...
#modified a bit from Julio's abcd workflow
...
#will need to add an extra step before the resample
    ...
    for session in sessions:
        ...
        for clean_subj_file in clean_subj_files:
            ...
            for i, roi in enumerate(rois):
                ...
                print(roi_name)
                #modified for rois w no "-"
                if '-' in roi_name:
                    roi_prefix = roi_name.split("_")[0].split("-")[1]
                else:
                    roi_prefix = "hb" #will change this but for now
